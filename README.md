...
- 15+ HUD modules: Totems, Coordinates, Armor Durability, FPS, Ping, Player Model, Inventory Viewer, TPS, IP, Time, Speed, Server Brand, Keystrokes, Potion Effects, Reach, TNT
...
- Customizable text formats with `{x}` `{Fps}` `{Ping}` `{Speed}` `{Tps}` `{Ip}` `{Time}` `{Brand}` `{Reach}` placeholders
...
Available modules: `TotemHud`, `CoordsHud`, `ArmorHud`, `FPS`, `Ping`, `PlayerModel`, `InventoryViewer`, `TPS`, `IP`, `Time`, `Speed`, `Brand`, `Keystrokes`, `PotionEffects`, `Reach`, `TntHud`
...
6. **TNT settings**: Right-click TntHud to toggle `Bar Mode` and adjust `Decimal Places`
